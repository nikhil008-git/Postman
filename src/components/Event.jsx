      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 my-6 sm:my-10 flex flex-col items-center justify-center">
        ...
        <div className="w-full mb-12">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div className="col-span-1 sm:col-span-2 lg:col-span-1">
              <img src={memory} alt="Event Memory" className="w-full h-64 object-cover rounded-lg shadow-lg" />
            </div>
            <div className="col-span-1">
              <img src={ghibli} alt="Event Ghibli" className="w-full h-64 object-cover rounded-lg shadow-lg" />
            </div>
            <div className="col-span-1">
              <img src={postmanGhibli} alt="Postman Ghibli" className="w-full h-64 object-cover rounded-lg shadow-lg" />
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center justify-center w-full">
          {events.length === 0 ? (
            ...
            events.map((event) => (
              <div key={event._id} className="bg-white text-black flex flex-col sm:flex-row justify-center items-center rounded-2xl mt-4 shadow-lg gap-2 m-2 w-full">
                <div className="bg-orange-400 text-black flex flex-col justify-center items-center p-4 sm:p-6 rounded-2xl w-full sm:w-auto mx-2">
                  <h4 className="text-sm sm:text-base font-medium">{new Date(event.date).getFullYear()}</h4>
                  <h1 className="text-2xl sm:text-3xl font-bold">{new Date(event.date).getDate()}</h1>
                  <h4 className="text-sm sm:text-base font-medium">{new Date(event.date).toLocaleString('default', { month: 'short' })}</h4>
                </div>
                <div className="h-auto sm:h-40 text-black flex flex-col justify-center items-start p-4 sm:p-6 border-b-2 sm:border-b-0 sm:border-r-2 border-gray-600 w-full">
                  <div className="font-bold text-sm sm:text-base mb-2">{event.title}</div>
                ...
                <div className="flex flex-col justify-center items-center p-4 sm:p-6 gap-4 sm:gap-6 w-full">
                  <div className="flex flex-col justify-center items-start sm:items-center px-4 sm:px-6 gap-2 w-full">
                    <div className="flex flex-row gap-2 items-center">
                      <CalendarDays className="w-4 h-4 sm:w-5 sm:h-5 text-black" />
                      <h2 className="text-xs sm:text-sm text-black">{event.time}</h2>
                    </div>
                    <div className="flex flex-row gap-2 items-center">
                      <TicketMinus className="w-4 h-4 sm:w-5 sm:h-5 text-black" />
                      <h2 className="text-xs sm:text-sm text-black">{event.availableSeats} seats available</h2>
                    </div>
                    <div className="flex flex-row gap-2 items-center">
                      <MapPin className="w-4 h-4 sm:w-5 sm:h-5 text-black" />
                      <h2 className="text-xs sm:text-sm text-black">{event.location}</h2>
                    </div>
                  </div>
